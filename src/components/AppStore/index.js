...
class AppStore extends Component {
  state = {tabId: tabsList[0].tabId, userInput: ''}

  filteredApps = () => {
    const {tabId, userInput} = this.state

    const filteredApps = appsList.filter(
      each => each.category.toUpperCase() === tabId.toUpperCase(),
    )
  ...
  selectedAppsList = filteredAppsList => {
    const {userInput} = this.state
    const userFilteredApps = filteredAppsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(userInput.toLowerCase()),
    )
    return userFilteredApps
  }

  render() {
    const {tabId, userInput} = this.state

    const filteredAppsList = this.filteredApps()

    const userFilteredApps = this.selectedAppsList(filteredAppsList)
